...
class Form extends React.Component {
  ...
  async componentDidMount() {
    ...
    await this.props.apolloClient.httpClient
      .query({
        query: GET_EVENTPICKTWO_AND_STICKER_CATEGORY,
      })
      .then((result) => {
        this._isMounted &&
          this.setState({
            eventPick: result.data.getEventPickTwo,
  ...
  async onHandleSubmit(event) {
    ...
    let promise = await new Promise((resolve) =>
      this.props.apolloClient.httpClient
        .mutate({
          mutation: SAVE_EVENTPICKTWO,
          variables: {
            eventPickTwo: eventPick,
          },
        })
        .then((result) => {
          if (result.data.saveEventPickTwo.statusCode === 200) {
            resolve(true);
          } else {
            resolve(false);
          }
    ...
    if (promise) {
      this._isMounted &&
        this.props.enqueueSnackbar(
          "Event pick two has been successfully updated.",
          { variant: "success" }
        );
    } else {
      this._isMounted &&
