...
import { addDays, subDays, format, differenceInDays, formatISO } from "date-fns";
...
const rangeLimitDay = 60;
const rangeLimitDayDefault = 30;

class TabDaily extends React.Component {
    /**
     * @constructor
     */
    constructor(props) {
        ...
        this.state = {
            pageUrl: '-1',
            startDate: subDays(new Date(formatISO(new Date(), { representation: 'date' })), rangeLimitDayDefault - 1),
            endDate: new Date(formatISO(new Date(), { representation: 'date' })),
            country: '-1',
            dataChart: [],
            dataTopCountries: [],
            listTopCountries: [],
    ...
    setDate(value, type) {
        ...
        if (type === "start") {
            const iDay = differenceInDays(this.state.endDate, date) + 1;
            // console.log('differenceInDays (start): ', iDay, date.toISOString(), this.state.endDate.toISOString());
            if (iDay <= rangeLimitDay) {
                this.setState({
                    startDate: date,
                });
            } else {
                this.setState({
                    startDate: date,
                    endDate: addDays(date, rangeLimitDay - 1),
                });
            }
        } else {
            const iDay = differenceInDays(date, this.state.startDate) + 1;
            // console.log('differenceInDays (end): ', iDay, this.state.startDate.toISOString(), date.toISOString());
            if (iDay <= rangeLimitDay) {
                this.setState({
                    endDate: date,
                });
                ...
                this.setState({
                    endDate: date,
                    startDate: subDays(date, rangeLimitDay - 1),
                });
            }
        }
    }
    ...
    getOptions() {
        const label = [...this.state.dataChart.map(item => format(new Date(item.timeframe), "yyyy-MM-dd"))];
    ...
    download() {
        ...
        getCSVStatistics(
            ["Country", "Average", ...(this.state.listTopCountries.length > 0 ? this.state.dataTopCountries.filter(day => day.country === this.state.listTopCountries[0].country).map(day => day.timeframe !== null ? format(new Date(day.timeframe), "yyyy-MM-dd") : '') : [])],
            aData,
                                ...
                                <TableHead>
                                    <TableRow>
                                        ...
                                        {this.state.listTopCountries.length > 0 ? this.state.dataTopCountries.filter(day => day.country === this.state.listTopCountries[0].country).map(day => <TableCell
                                            key={day.timeframe}>{day.timeframe !== null ? format(new Date(day.timeframe), "yyyy-MM-dd") : ''}</TableCell>) : <></>}
                                    </TableRow>
                                </TableHead>
                                <TableBody>
...
export default withSnackbar(connect(mapStateToProps, null)(TabDaily));
