...
class GoogleAnalytics extends React.Component {
    ...
    async createDailyRequestByCountry(countries, labels) {
        ...
        for (var i = 0; i < labels.length; i++) {
            ...
            allDays.push(hour)
            let dailyCountries = await queryReport({...requestCountry, startDate: this.state.startDate, endDate: this.state.endDate, filter:"ga:hour==" + hour})
            ...
            dailyCountries.forEach(country => {
                ...
                if (cIndex >= 0) {
                    countries[cIndex][`${hour}`] = country.views
    ...
    download() {
        ...
        getCSV(["Nation", ...this.state.allDays], this.state.dateBasedCountries, "report-by-hour.csv", [[], ["", ...this.state.reportData.labels], ["", ...this.state.reportData.values]]);
