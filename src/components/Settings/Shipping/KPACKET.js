 ...
 * @package Settings
 */
class KPACKET extends React.Component {
  ...
  async componentDidMount() {
    ...
    let url = process.env.REACT_APP_DOMAIN + "/getKPACKET";
    let response = [];
  ...
  async onUpload(files) {
    ...
    let url = process.env.REACT_APP_DOMAIN + "/import/kpacket";
    ...
    let promise = await new Promise(resolve => axios({
        ...
        if(_response.data === "FAILURE") {
          ...
          await axios({
            ...
            method: 'GET',
            url: process.env.REACT_APP_DOMAIN + "/getKPACKET",
            responseType: 'json'
          }).then(_response => {
            resolve({
              success: true,
  ...
  async onDownload() {
    ...
    const url = process.env.REACT_APP_DOMAIN + "/download/KPACKET";
    ...
    await axios({
      ...
      fileDownload(response.data, "KPACKET.xlsx") ;
    }).catch(error => {
      ...
      <div style={{width: '100%', height: '700px', maxHeight: '700px', overflow: "auto"}}>
        <Table>
          <TableBody>
            ...
            {((this.state.packet[0] || {}).datas || []).map((data, index) => {
              return <TableRow key={index}>
                <TableCell>{data.weight}</TableCell>
                {(this.state.packet || []).map((country, index) => {
...
export default withSnackbar(KPACKET);