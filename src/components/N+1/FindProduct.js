...
class FindProduct extends React.Component {
  ...
  onSelectId(event, id) {
    ...
    console.log("선택된 상품들 : " , this.state.selectedProducts);
        ...
        this.state.isFetching ? <div className="customPopup"><LinearProgress/></div> : 
          <CardContent>
                      ...
                      <TableRow key={item.id}>
                        <TableCell padding="checkbox">
                          <Checkbox
                            color="primary"
                            // checked={
                            //   this.state.selectedProducts.indexOf(item.id) !==
                            //   -1
                            // }
            ...
            <Grid container className="mt-20">
              <Grid item xs={12} >
                <PaginationMaterial 
                count={Math.ceil(this.state.products.totalElements / this.state.pagination.rowsPerPage)} 
                page={this.state.pagination.pageNumber} 
                onChange={this.handlePageNumber} color="primary" 
